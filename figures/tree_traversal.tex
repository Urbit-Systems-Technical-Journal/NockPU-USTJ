...
\end{tikzpicture}
\caption{Stackless Tree Traversal Sequence demonstrating breadcrumb-based navigation without a traditional call stack. The NockPU implements tree traversal without a stack by modifying pointers to create breadcrumb trails. Sequence: (A) Initial tree state with root and subtrees. (B) Descend into the left subtree leaving breadcrumb. (C) Follow breadcrumb back to root to continue with right.}
